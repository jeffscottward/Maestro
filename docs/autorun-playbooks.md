...
| **Automatically create PR when complete** | When checked, Maestro opens a pull request from the worktree branch when the run completes |

This is the recommended workflow for longer Auto Runs when your current checkout is already busy. Your current branch stays untouched, isolated experiments land on dedicated worktree branches, and you get a PR at the end ready for review.
